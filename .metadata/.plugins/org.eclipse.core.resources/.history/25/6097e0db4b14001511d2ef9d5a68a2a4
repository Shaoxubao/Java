...
<body bgcolor="#CCCC99">
    ...
    <table width="1600" align="center" border="1">
      <tr>
        ...
        <td>业务管理室审签收到日期</td><td>业务管理室审签发出日期</td>
        <td>受权签字人审签收到日期</td><td>受权签字人审签发出日期</td>
        <td>最终检验报告收到日期</td><td>最终检验报告发出日期</td>
        <td>校对收到日期</td><td>校对发出日期</td>
        <td>打印收到日期</td><td> 打印发出日期</td><td>检验</td>
      </tr>
      <c:if test="${pagers.total le 0 }">
        <tr>
          <td colspan="19">目前还没有用户数据</td>
        </tr>
      </c:if>
      ...
      <c:if test="${pagers.total gt 0 }">
          <c:forEach items="${pagers.datas }" var="s">
            <tr>
              ...
              <td>${s.printRev }</td>
              <td>${s.printSend }</td>
              <td><a href="${s.sampleNo }/sampleflowcommit">检验</a></td>
            </tr>
          </c:forEach>
            <tr>
              <td colspan="19">
